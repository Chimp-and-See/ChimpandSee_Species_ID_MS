...
class.data<-read.table("your path here/all_raw_classification_data_Species_ID_MS.txt", sep="\t", header=T)
nrow(class.data)
#369671
length(unique(class.data$Video.ID))
#13531

expert.data<-read.table("your path here/all_expert_data_Species_ID_MS.txt", sep="\t", header=T)
nrow(expert.data)
#13814
...
############################################################
# step 1 - remove expert videos 

######## unidentifiable
#remove videos that contain at least one unidentifiable species
table(expert.data$Species)
uid.agg<-aggregate(expert.data$Species, list(expert.data$Video.ID), function(x){
    sum(x=="unidentifiable")
})

expert.data<-expert.data[!expert.data$Video.ID %in% uid.agg$Group.1[uid.agg$x>0],]
nrow(expert.data)
#13756
length(unique(expert.data$Video.ID))
#13476
...
expert.ms.table<-table(expert.data$Video.ID)
table(expert.ms.table)
#     1     2     3 
# 13212   248    16

#there are 264 expert multispecies videos at this stage
expert.data<-expert.data[expert.data$Video.ID %in% names(expert.ms.table[expert.ms.table==1]),]
nrow(expert.data)
#13212
length(unique(expert.data$Video.ID))
#13212
...
####### remove videos the expert classified only as duiker
table(expert.data$Species)
expert.data<-expert.data[expert.data$Species!="duiker.sp",]
nrow(expert.data)
#13095
#removed 117 videos with duiker classifications

##### make sure classification data only has the same videos present
class.data<-class.data[class.data$Video.ID %in% expert.data$Video.ID,]
nrow(class.data)
#355199
length(unique(class.data$Video.ID))
#13095
...
expert.data<-expert.data[!expert.data$Video.ID %in% buggy.clips$Video.ID,]
nrow(expert.data)
#12939
...
expert.data<-expert.data[!expert.data$Video.ID %in% short.blank.clips$Video.ID,]
nrow(expert.data)
#11957
...
expert.data<-expert.data[expert.data$Video.ID %in% names(clip.table[clip.table==4]),]
nrow(expert.data)
#11896
